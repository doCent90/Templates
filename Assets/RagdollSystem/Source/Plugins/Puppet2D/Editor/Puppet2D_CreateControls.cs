...
namespace Puppet2D
{
	public class Puppet2D_CreateControls : Editor
	{
		...
		public static void IKCreateTool(bool worldSpace = false)
		{
			...
			if (bone)
			{
				if (!bone.GetComponent<Puppet2D_Bone>())
				{
			...
			if (bone.transform.parent && bone.transform.parent.transform.parent && bone.transform.parent.transform.parent.GetComponent<Puppet2D_Bone>() )
			...
			globalCtrl.GetComponent<Puppet2D_GlobalControl>().FlipCorrection = -1;
