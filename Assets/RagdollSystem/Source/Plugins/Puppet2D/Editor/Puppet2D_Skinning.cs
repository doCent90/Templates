	...
	public class Puppet2D_Skinning : Editor
	{
		...
		public static void BindRigidSkin(List<GameObject> selectedBones = null, List<GameObject> selectedMeshes = null)
		{
			if (selectedMeshes == null)
			{
				...
				{
					if (Obj.GetComponent<Puppet2D_Bone>() != null)
					{
		...
		public static GameObject BindSmoothSkin(int isGeosedic = 0)
		{
			...
			foreach (GameObject Obj in selection)
			{
				if (Obj.GetComponent<SpriteRenderer>() && Obj.GetComponent<SpriteRenderer>().sprite && Obj.GetComponent<SpriteRenderer>().sprite.name.Contains("ffdBone"))
				{
					ffdControls.Add(Obj);
					selectedBones.Add(Obj.transform);
				}
				else if (Obj.GetComponent<Puppet2D_Bone>() != null)
				{
					selectedBones.Add(Obj.transform);
				}
				else if ((Obj.GetComponent<MeshRenderer>()) || (Obj.GetComponent<SkinnedMeshRenderer>()))
				{					
					selectedMeshes.Add(Obj);					
				...
				{
					Debug.LogWarning("Please some Puppet2D bones and a GameObject with a MeshRenderer");
					//return null;
				}
			}
			...
			if ((ffdControls.Count > 0) && selectedMeshes.Count == 0 && ffdControls[0].GetComponent<Puppet2D_FFDLineDisplay>().outputSkinnedMesh)
			{
				...
				{
					if(tr.gameObject.GetComponent<Puppet2D_Bone>())
		...
		*/
		...
		public static void SortVertices(GameObject gameObject = null)
		{
			...
			Puppet2D_Bone[] bones = new Puppet2D_Bone[smr.bones.Length];
			for (int c = 0; c < bones.Length; c++)
				bones[c] = smr.bones[c].GetComponent<Puppet2D_Bone>();
		...
		public static int[] BubbleSort(int[] tris, Puppet2D_Bone[] bones, Mesh mesh)
		{
			...
			for (j = N - 1; j > 0; j--)
			{
				for (i = 0; i < j; i++)
				{
					if (bones[mesh.boneWeights[tris[i * 3]].boneIndex0].Order > bones[mesh.boneWeights[tris[((i * 3) + 3)]].boneIndex0].Order)
					{
		...
		static void EuclidianSkinWeights(GameObject mesh, List<Transform> selectedBones, List<GameObject> ffdControls)
		{
			...
			for (int j = 0; j < weights.Length; j++)
			{
				...
				for (int i = 0; i < selectedBones.Count; i++)
				{
					...
					Vector2 centrePoint = new Vector2(selectedBones[i].transform.position.x, selectedBones[i].transform.position.y);
					if (selectedBones[i].transform.childCount > 0)
					{
						centrePoint = 0.5f * (centrePoint + new Vector2(selectedBones[i].transform.GetChild(0).transform.position.x, selectedBones[i].transform.GetChild(0).transform.position.y));
					}
					float dist = Vector2.Distance(centrePoint, new Vector2(worldPt.x, worldPt.y));
					//float dist = Vector2.Distance(new Vector2(selectedBones[i].transform.position.x, selectedBones[i].transform.position.y), new Vector2(worldPt.x, worldPt.y));
					if (dist < testdist)
					{
						testdist = dist;
		...
		public static Mesh SaveFBXMesh(Mesh mesh, bool Duplicate = false)
		{
			...
			else
			{
				...
				string outMeshPath = meshPath + mesh.name + "P2D.asset";
