...
namespace RagdollSystem
{
    ...
    public partial class PhysicleMaterialsHandler : IPhysicleMaterialsHandler
    {
        ...
        private void SetTemperature(PhysicleMaterial material)
        {
            ...
            float speed = TemperatureValue < 0 ? SpeedMultiplierFactor : _defaultAnimationSpeed;

            if (_animator != null)
                _animator.OnSpeedChanged(speed);

            if (_character != null)
                _character.MovementState.OnSpeedFactorChanged(speed);
        }
    }
}
