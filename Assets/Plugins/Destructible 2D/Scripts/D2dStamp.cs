...
namespace Destructible2D
{
	...
	public static class D2dStamp
	{
		...
		public static void All(D2dDestructible.PaintType paint, Vector2 position, Vector2 size, float angle, Texture2D shape, Color color, int layerMask = -1, List<GameObject> exclusions = default)
		{
			All(paint, CalculateMatrix(position, size, angle), shape, color, layerMask, exclusions);
		}

		public static void All(D2dDestructible.PaintType paint, Matrix4x4 matrix, Texture2D shape, Color color, int layerMask = -1, List<GameObject> exclusions = default)
		{
			var destructible = D2dDestructible.FirstInstance;
			exclusions ??= new List<GameObject>();

			for (var i = D2dDestructible.InstanceCount - 1; i >= 0; i--)
			{
                if (exclusions.Count > 0 && exclusions.Contains(destructible.gameObject)) continue;
